...
const OfflineGuestsPage = () => {
    const { guestList, addGuestToList, editGuest, deleteGuest, loading } = useOfflineGuests();
    const { theme } = useTheme();
    const isDarkMode = theme === 'dark';
    const { updateRoomStatus, getRooms } = useOfflineRoomReservation();
    ...
    const addDashboardNotification = (notification) => {
        try {
            console.log('Offline Notification:', notification);
            
            // Dispatch occupancy update event to refresh room status displays
    ...
    useEffect(() => {
        const savedActivities = localStorage.getItem('offline_guest_activities');
        if (savedActivities) {
    ...
    const addActivity = (activity) => {
        try {
            ...
            localStorage.setItem('offline_guest_activities', JSON.stringify(updatedActivities));
            
            // Add to dashboard notifications for important activities
    ...
    // Add guest wrapper to include notifications
    const handleAddGuest = async (guestData) => {
        try {
            const addedGuest = await addGuestToList(guestData);
            ...
            if (addedGuest) {
                ...
                if (guestData.room) {
                    await updateRoomStatus(guestData.room, 'Occupied');
                    toast.success(`Guest ${guestData.name} added to ${guestData.room_name || 'room'}`);
                }
    ...
    const handleEditSubmit = async (guestId, updatedData) => {
        try {
            const originalGuest = guestList.find(g => g.id === guestId);
            const updatedGuest = await editGuest(guestId, updatedData);
            ...
            if (originalGuest && updatedGuest && originalGuest.room !== updatedGuest.room) {
                ...
                if (originalGuest.room) {
                    await updateRoomStatus(originalGuest.room, 'Available');
                }
                
                // If new room exists, mark it as occupied
                if (updatedGuest.room) {
                    await updateRoomStatus(updatedGuest.room, 'Occupied');
                }
        ...
        if (selectedGuest) {
            try {
                ...
                if (selectedGuest.room) {
                    await updateRoomStatus(selectedGuest.room, 'Available');
                }
    ...
    const handleCheckIn = async (guestId) => {
        try {
            ...
            if (guest) {
                ...
                await editGuest(guestId, updatedData);
                
                // Update room status to Occupied
                if (guest.room) {
                    await updateRoomStatus(guest.room, 'Occupied');
                }
                
                addActivity(`${guest.name} checked into ${guest.room_name || 'their room'}`);
                
                // Add dashboard notification
                addDashboardNotification({
                    type: 'guest_checkin',
                    title: 'Guest Check-in',
                    message: `${guest.name} has checked into ${guest.room_name || 'their room'}.`,
                    time: new Date().toISOString()
                });
    ...
    const handleCheckOut = async (guestId) => {
        try {
            ...
            if (guest) {
                ...
                if (guest.room) {
                    await updateRoomStatus(guest.room, 'Available');
                }
                
                addActivity(`${guest.name} checked out from ${guest.room_name || 'their room'}`);
                
                // Add dashboard notification
                addDashboardNotification({
                    type: 'guest_checkout',
                    title: 'Guest Check-out',
                    message: `${guest.name} has checked out from ${guest.room_name || 'their room'}.`,
                    time: new Date().toISOString()
                });
    ...
    if (loading) {
        return (
            <div className={`flex h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
                <OfflineSidebar activeLink="Guests" />
                <div className="flex-1 overflow-auto">
                    <OfflineNavbar title="Guest Management" />
                    <div className="p-6 flex items-center justify-center">
                        <div className="text-center">
                            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-green-500 mx-auto mb-4"></div>
                            <p className={`${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Loading guests...</p>
                        </div>
                    </div>
                </div>
            </div>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex items-center">
                                                        <div className={`h-10 w-10 rounded-full bg-gradient-to-br ${
                                                            ...
                                                            {guest.name ? guest.name.split(' ').map(n => n[0]).join('') : 'G'}
                                                        </div>
                                                        <div className="ml-4">
                                                            <div className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>{guest.name || 'Unknown'}</div>
                                                        </div>
                                                    </div>
                                                </td>
            ...
            {isViewModalOpen && selectedGuest && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg p-6 max-w-md w-full max-h-96 overflow-y-auto`}>
                        <h3 className={`text-lg font-bold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Guest Details</h3>
                        <div className="space-y-2">
                            ...
                            <p><strong>Room:</strong> {selectedGuest.room_name || selectedGuest.room || 'N/A'}</p>
                            <p><strong>Status:</strong> {selectedGuest.status || 'N/A'}</p>
                            <p><strong>Check In:</strong> {formatDate(selectedGuest.checkIn || selectedGuest.check_in)}</p>
                            <p><strong>Check Out:</strong> {formatDate(selectedGuest.checkOut || selectedGuest.check_out)}</p>
                        </div>
            ...
            {isEditModalOpen && selectedGuest && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg p-6 max-w-md w-full`}>
                        <h3 className={`text-lg font-bold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Edit Guest</h3>
...
export default OfflineGuestsPage; 