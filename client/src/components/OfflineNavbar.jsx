...
import { faBell, faUserCircle, faCog, faSignOutAlt, faSearch, faShieldAlt, faPlugCircleXmark } from '@fortawesome/free-solid-svg-icons';
import { useOfflineUser } from '../context/OfflineUserContext';
import { useTheme } from '../context/ThemeContext';
import { useNavigate } from 'react-router-dom';

const OfflineNavbar = ({ title }) => {
        ...
        <header className={`${isDarkMode ? 'bg-gray-900/80' : 'bg-white/90'} backdrop-blur-md border-b mb-8 sticky top-0 z-40 ${isDarkMode ? 'border-gray-700/50' : 'border-gray-200/70'} transition-all duration-300`}>
            <div className="px-6 py-3 flex items-center justify-between">
                <div className="flex items-center">
                    <div className={`mr-3 flex items-center justify-center w-9 h-9 rounded-full ${isDarkMode ? 'bg-green-500' : 'bg-green-600'} text-white`}>
                        <FontAwesomeIcon icon={faPlugCircleXmark} className="text-sm" />
                    </div>
                    <h1 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-800'} flex items-center`}>
                        ...
                        <span className={`ml-2 px-2 py-1 text-xs rounded-full ${isDarkMode ? 'bg-green-800 text-green-200' : 'bg-green-100 text-green-800'}`}>
                            OFFLINE
                        </span>
                    </h1>
                </div>
                ...
                <div className={`hidden md:flex items-center mx-4 px-4 py-2 flex-1 max-w-md rounded-full ${isDarkMode ? 'bg-gray-800/70 text-gray-300' : 'bg-gray-100/80 text-gray-600'} border ${isDarkMode ? 'border-gray-700/50' : 'border-gray-200/50'}`}>
                    ...
                    <input 
                        type="text" 
                        placeholder="Search (Offline Mode)..." 
                ...
                <div className="flex items-center space-x-5">
                    ...
                    <button className="relative group">
                        <div className={`p-2 rounded-full transition-all duration-300 ${isDarkMode ? 'group-hover:bg-gray-800' : 'group-hover:bg-gray-100'}`}>
                            ...
                            <span className="absolute -top-1 -right-1 bg-gradient-to-r from-green-500 to-emerald-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center shadow-lg shadow-green-500/20">0</span>
                        </div>
                    </button>
                    ...
                    <div className="relative">
                        ...
                        >
                            {avatarUrl ? (
                                ...
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isDarkMode ? 'bg-gray-700' : 'bg-green-100'}`}>
                                    <FontAwesomeIcon icon={faUserCircle} className={`${isDarkMode ? "text-gray-300" : "text-green-500"} text-lg`} />
                                </div>
                            )}
                        ...
                        {isUserMenuOpen && (
                            <div 
                                className={`absolute right-0 mt-2 w-52 py-2 rounded-xl shadow-lg shadow-green-500/10 z-50 backdrop-blur-md ${isDarkMode ? 'bg-gray-800/95 text-white' : 'bg-white/95 text-gray-800'} ring-1 ${isDarkMode ? 'ring-gray-700' : 'ring-gray-200'} overflow-hidden border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}
                            ...
                            >
                                <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                                    ...
                                    <p className={`text-xs ${isDarkMode ? 'text-green-400' : 'text-green-600'} font-medium mt-1`}>
                                        🔴 Offline Mode
                                    </p>
                                </div>
                                <button
