  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newNotification = {
        type: serviceRequest ? 'info' : 'success',
        title: serviceRequest ? 'Service Request Updated' : 'New Service Request',
        message: `${serviceRequest ? 'Updated' : 'New'} request for ${formData.serviceName} in Room ${formData.room}`,
        time: 'Just now',
      ...
      const localRequest = {
        id: serviceRequest ? serviceRequest.id : Date.now(),
        ...formData,
        service_id: formData.serviceId ? parseInt(formData.serviceId) : null,
        service_name: formData.serviceName,
        requested_by: formData.requestedBy,
      ...
      // Get existing local service requests
      const localRequestsStr = localStorage.getItem('hotelServiceRequests');
      const localRequests = localRequestsStr ? JSON.parse(localRequestsStr) : [];
      
      if (serviceRequest) {
        // Update existing service request
        const updatedRequests = localRequests.map(r => 
          r.id === serviceRequest.id ? localRequest : r
        );
        localStorage.setItem('hotelServiceRequests', JSON.stringify(updatedRequests));
      } else {
        // Add new service request
        localRequests.push(localRequest);
        localStorage.setItem('hotelServiceRequests', JSON.stringify(localRequests));
      }
      
      // Still call onSave with the local service request
      if (onSave) onSave(localRequest);
